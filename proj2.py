...
def print_processing_time(start_time):
    time_str = "[... %s seconds]" % round((time.time() - start_time), 2)
...
def get_filter_day():
    ...
    while True:
        ...
        if day == 'a':
            ...
            day = WEEKDAYS[int(day) - 1]    # here we did -1 to get the correct index
            break
        else:
            continue
...
def display_raw_data(df):
    ...
    print('\n    do you like to see some raw data from the current dataset?')
    while True:
